...
- (instancetype)initWithTitle:(NSString *)title message:(NSString*)msg url:(NSString*)profileURL duration:(CGFloat)duration completion:(void (^)(void))completion {
...
- (void)setUpAlertView {
    ...
    [alertView setBackgroundColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:1]];
}
...
- (void)setUpTitleLabel {
    ...
    if (!titleLabel) {
        ...
        titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(60, rect.origin.y , rect.size.width - 60, 20)];
        
        profileImage = [[UIImageView alloc] initWithFrame:CGRectMake(10, 20 , 40, 40)];
        profileImage.layer.cornerRadius = 20;
        profileImage.layer.masksToBounds = YES;
        messageLabel=[[UILabel alloc]initWithFrame:CGRectMake(60, rect.origin.y +20, rect.size.width - 60, 20)];
        
        NSURL *url = [[NSURL alloc]initWithString: profile_url];
        
        profileImage.image = [[UIImage alloc] initWithData:[[NSData alloc]initWithContentsOfURL:url]];
        alertView.layer.cornerRadius = 5;
        alertView.layer.masksToBounds = YES;
        alertView.layer.borderColor = (__bridge CGColorRef _Nullable)([UIColor blackColor]);
        alertView.layer.borderWidth = 1;
        
        [messageLabel setText:msgString];
        [messageLabel setTextAlignment:NSTextAlignmentLeft];
        [messageLabel setTextColor:[UIColor blackColor]];
        [messageLabel setFont:[UIFont fontWithName:@"Muli-Regular" size:15.0]];
        [messageLabel setMinimumScaleFactor:14.0/16.0];
        [titleLabel setText:titleString];
        [titleLabel setTextAlignment:NSTextAlignmentLeft];
        
        [titleLabel setTextColor:[UIColor colorWithRed:97.0/255.0 green:18.0/255.0 blue:101.0/255.0 alpha:1]];
        [titleLabel setFont:[UIFont fontWithName:@"Muli-Bold" size:18.0]];
        ...
        if (size.width > titleLabel.bounds.size.width) {
            ...
            [titleLabel setFrame:CGRectMake(30, rect.origin.y + 50, size.width, 60)];
            [scrollView addSubview:titleLabel];
        }
        else {
            [alertView addSubview:titleLabel];
            [alertView addSubview:messageLabel];
            [alertView addSubview:profileImage];
        }
...
- (void)slideScrollView {
    ...
    [UIScrollView setAnimationDuration:timeDuration/1.2];
...
- (void)setAlertType:(AlertType)alertType {
    ...
    //red: 97.0/255.0, green: 18.0/255.0, blue: 101.0/255.0, alpha: 1
...
- (void)configureIncomingAnimationFor:(AlertIncomingTransitionType)trannyType {
    ...
    switch (trannyType) {
        case AlertIncomingTransitionTypeFlip: {
            ...
            if (doesBounce) {
                ...
                [UIView animateWithDuration:0.185 animations:^{
                    [self->alertView setFrame:[self alertRect]];
                }];
        ...
        case AlertIncomingTransitionTypeInYoFace: {
            ...
            [UIView animateWithDuration:0.3 delay:0.0 usingSpringWithDamping:0.95 initialSpringVelocity:0.9 options:UIViewAnimationOptionTransitionNone animations:^{
    ...
    switch (trannyType) {
        case AlertOutgoingTransitionTypeFlip: {
            ...
            [UIView animateWithDuration:0.275 animations:^{
                self->alertView.transform = CGAffineTransformMake(1, 0, 0, -0.05 , 0, self->alertView.transform.ty - 18.5);
            } completion:^(BOOL finished) {
        ...
        case AlertOutgoingTransitionTypeSlideToTop: {
            ...
            [UIView animateWithDuration:0.15 animations:^{
                [self->alertView setFrame:finalRect];
            } completion:^(BOOL finished) {
                [self finishDisappearing];
            }];
        ...
        case AlertOutgoingTransitionTypeOutYoFace: {
            ...
            [UIView animateWithDuration:0.25 delay:0.0 usingSpringWithDamping:0.95 initialSpringVelocity:0.9 options:UIViewAnimationOptionTransitionNone animations:^{
                self->alertView.transform = CGAffineTransformMakeScale(1.25, 1.25);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:0.15 animations:^{
...
- (CGRect)alertRect {
    ...
    return CGRectMake(0, 20, mainScreen.bounds.size.width + 0, 70);
}
